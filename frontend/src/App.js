...
const Home = () => {
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener("scroll", onScroll); window.removeEventListener("mousemove", onMouse); observer.disconnect(); cancelAnimationFrame(frame); lenis.destroy(); };
  }, []);
  ...
  return (
    <main>
      ...
      <section className="hero-immersion" id="hero-immersion" data-testid="hero-section"><div className="hero-stage"><div className="hero-image" data-testid="hero-immersion-image"></div><div className="hero-grain"></div><div className="hero-orbit orbit-one"></div><div className="hero-orbit orbit-two"></div><div className="hero-overlay"></div><div className="hero-copy"><p className="eyebrow" data-testid="hero-eyebrow">Digital growth systems / 001 — 2025</p><h1 data-testid="hero-heading">Make growth<br /><em>repeatable.</em></h1><p className="hero-sub" data-testid="hero-description">We combine AI automation, deployed agents, and high-converting websites to turn ambitious businesses into faster-moving companies.</p></div><div className="hero-glass glass-coordinate" data-testid="hero-coordinate-card"><span>Automation</span><span>Agents</span><small>WE BUILD THE LAYER BETWEEN YOU AND SCALE</small></div><div className="hero-focus" data-testid="hero-focus-point"></div><div className="hero-foot"><span data-testid="hero-scroll-label">Scroll to see the system</span><span>© ASCENXION 2025</span><span>Built for the next</span></div></div></section>
      <section className="statement" data-testid="statement-section"><p className="eyebrow reveal">The growth gap</p><h2 className="reveal" data-testid="statement-heading">Stop adding<br />more work.<br /><em>Build leverage.</em></h2><p className="statement-copy reveal">Your next stage of growth should not depend on more tabs, more tools, or more people chasing the same bottlenecks. Ascenxion designs the digital systems that make momentum compound.</p><div className="offer-strip" data-testid="offer-strip"><div className="reveal"><span>01 / AUTOMATE</span><strong>Give your team back its time.</strong><small>AI workflows that remove repetitive work and keep revenue moving.</small></div><div className="reveal"><span>02 / DEPLOY</span><strong>Put an agent on the front line.</strong><small>Purpose-built AI agents that support customers, sales, and operations.</small></div><div className="reveal"><span>03 / CONVERT</span><strong>Make every visit count.</strong><small>High-class websites engineered for clarity, trust, and action.</small></div></div><div className="story-line" data-testid="story-line"><div className="story-step active reveal"><span>01</span><strong>Find the friction</strong><small>See where time, attention, and revenue are leaking.</small></div><div className="story-step reveal"><span>02</span><strong>Build the leverage</strong><small>Turn the bottleneck into a system that works.</small></div><div className="story-step reveal"><span>03</span><strong>Scale the signal</strong><small>Make the advantage visible to every customer.</small></div></div><div className="stat-row reveal"><div><strong>24<span>+</span></strong><small>Systems shipped</small></div><div><strong>08</strong><small>Industries transformed</small></div><div><strong>∞</strong><small>Room to scale</small></div></div></section>
      <section className="work-section" id="work" data-testid="work-section"><div className="section-head reveal"><div><p className="eyebrow">Selected systems / 004</p><h2>Work that<br /><em>moves</em> markets.</h2></div><p className="section-note">A small selection of the systems we've designed for teams thinking in decades, not quarters.</p></div><div className="project-grid">{projects.map((project, index) => <article className={`project-card card-${index + 1} reveal`} key={project.number} data-testid={`project-card-${index + 1}`}><div className="project-image" style={{ backgroundImage: `url(${project.image})` }}></div><div className="project-shade"></div><div className="project-meta"><span>{project.number} / 2025</span><ArrowUpRight size={18} /></div><div className="project-title"><p>{project.type}</p><h3>{project.title}</h3><strong>{project.metric}</strong><small>{project.detail}</small></div></article>)}</div></section>
      <section className="capabilities" id="capabilities" data-testid="capabilities-section"><div className="cap-head reveal"><p className="eyebrow">What we build</p><h2>Intelligence,<br /><em>engineered.</em></h2></div><div className="service-list">{services.map((service, index) => <div className="service reveal" key={service} data-testid={`service-item-${index + 1}`}><span>0{index + 1}</span><h3>{service}</h3><ArrowUpRight size={20} /></div>)}</div></section>
      <section className="contact-section" id="contact" data-testid="contact-section"><div className="contact-art"></div><div className="contact-inner reveal"><p className="eyebrow">Have a system in mind?</p><h2>Let’s build<br /><em>what’s next.</em></h2><p>Tell us where you're going. We'll show you the shortest path to get there.</p>{submitted ? <div className="success-message" data-testid="inquiry-success"><Check size={22} /> Received. We’ll be in touch shortly.</div> : <form onSubmit={submitInquiry} data-testid="inquiry-form"><input required placeholder="Your name" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} data-testid="inquiry-name-input" /><input required type="email" placeholder="Work email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} data-testid="inquiry-email-input" /><input placeholder="Company (optional)" value={form.company} onChange={e => setForm({ ...form, company: e.target.value })} data-testid="inquiry-company-input" /><textarea required placeholder="What are you looking to build?" value={form.project} onChange={e => setForm({ ...form, project: e.target.value })} data-testid="inquiry-project-input" /><button type="submit" className="submit-button" data-testid="inquiry-submit-button">Start the conversation <Send size={17} /></button></form>}<a className="email-link" href="mailto:hello@ascenxion.ai" data-testid="email-cta">Prefer email? hello@ascenxion.ai <ArrowUpRight size={16} /></a></div></section>
