...
const Home = () => {
  ...
  useEffect(() => {
    const onScroll = () => document.documentElement.style.setProperty("--scroll-progress", `${Math.min(window.scrollY / 700, 1)}`);
  ...
  return (
    <main>
      ...
      <section className="hero" id="top" data-testid="hero-section"><div className="hero-image" data-testid="hero-immersion-image"></div><div className="hero-overlay"></div><div className="hero-copy"><p className="eyebrow" data-testid="hero-eyebrow">AI systems / 001 — 2025</p><h1 data-testid="hero-heading">Make your<br /><em>next move</em><br />intelligent.</h1><p className="hero-sub" data-testid="hero-description">We design and deploy production-grade AI systems that turn ambitious businesses into category leaders.</p><button className="circle-cta" onClick={() => goTo("work")} data-testid="hero-explore-button"><MoveDown size={18} /> <span>Explore<br />the work</span></button></div><div className="hero-foot"><span data-testid="hero-scroll-label">Scroll to connect</span><span>© ASCENXION 2025</span><span>Built for the next</span></div></section>
