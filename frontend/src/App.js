...
const indexItems = [
  { label: "Work", id: "work", numeral: "I" },
  { label: "Capabilities", id: "capabilities", numeral: "II" },
  { label: "Process", id: "process", numeral: "III" },
  { label: "Contact", id: "contact", numeral: "IV" }
];
...
const Home = () => {
  ...
  return (
    <main id="top">
      ...
      <section className="hero-immersion" id="hero-immersion" data-testid="hero-section"><div className="hero-stage"><div className="hero-image" data-testid="hero-immersion-image"></div><div className="hero-grain"></div><div className="hero-orbit orbit-one"></div><div className="hero-orbit orbit-two"></div><div className="hero-overlay"></div><div className="accent accent-orb"><div className="accent-inner orb-inner"></div></div><div className="accent accent-ring"><div className="accent-inner ring-inner"></div></div><div className="accent accent-prism"><div className="accent-inner prism-inner"></div></div><motion.div className="hero-panel" data-testid="hero-panel" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.7, delay: 0.15 }}><motion.p className="eyebrow" data-testid="hero-eyebrow" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.9, delay: 0.35 }}>Digital growth systems / 001 — 2026</motion.p><h1 data-testid="hero-heading"><MaskedLine delay={0.45}>Make growth</MaskedLine><MaskedLine delay={0.62}><em>repeatable.</em></MaskedLine></h1><motion.p className="hero-sub" data-testid="hero-description" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.95, ease: EASE }}>We combine AI automation, deployed agents, and high-converting websites to turn ambitious businesses into faster-moving companies.</motion.p><motion.div className="hero-index" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 1.2 }}><div className="index-links">{indexItems.map((item, i) => <button key={item.id} onClick={() => goTo(item.id)} data-testid={`hero-index-${item.id}`}><span>0{i + 1}</span>{item.label}</button>)}</div><div className="index-numerals">{indexItems.map(item => <span key={item.id}>{item.numeral}</span>)}</div></motion.div></motion.div><div className="hero-glass glass-coordinate" data-testid="hero-coordinate-card"><span>Automation</span><span>Agents</span><small>WE BUILD THE LAYER BETWEEN YOU AND SCALE</small></div><div className="hero-focus" data-testid="hero-focus-point"></div><div className="hero-foot"><span data-testid="hero-scroll-label">Scroll to see the system</span><span>© ASCENXION 2026</span><span>Built for the next</span></div></div></section>
